...
export default function AccessoriesandPerfumes() {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          "http://localhost:5000/api/products?category=woman_fashion&sub_category=Accessories and Perfumes"
        );
        const data = await res.json();
        setProducts(data);
      } catch (error) {
        console.error("Error fetching Accessories and Perfumes:", error);
      }
    };
    fetchProducts();
  }, []);
