...
export default function GirlsFashion() {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          "http://localhost:5000/api/products?category=kids&sub_category=Girls Fashion"
        );
        const data = await res.json();
        setProducts(data);
      } catch (error) {
        console.error("Error fetching Girls Fashion:", error);
      }
    };
    fetchProducts();
  }, []);
