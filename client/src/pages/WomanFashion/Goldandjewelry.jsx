...
export default function Goldandjewelry() {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          "http://localhost:5000/api/products?category=woman_fashion&sub_category=Gold and jewelry"
        );
        const data = await res.json();
        setProducts(data);
      } catch (error) {
        console.error("Error fetching Gold and jewelry:", error);
      }
    };
    fetchProducts();
  }, []);
