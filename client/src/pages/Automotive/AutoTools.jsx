// src/pages/Automotive/AutoTools.jsx
import React, { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import "../../styles/Cameras.css";

export default function AutoTools() {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          "http://localhost:5000/api/products?category=automotive&sub_category=Auto Tools"
        );
        const data = await res.json();
        setProducts(data);
      } catch (error) {
        console.error("Error fetching AutoTools:", error);
      }
    };
    fetchProducts();
  }, []);
  ...
  return (
    <div className="page-container">
      ...
      <aside className="sidebar-left">
        <h3>Automotive</h3>
        <ul>
          <li><Link to="/Automotive/AutoTools">AutoTools</Link></li>
          <li><Link to="/Automotive/CarsParts">CarsParts</Link></li>
      ...
      <main className="main-content">
        <h2>Bullet AutoTools</h2>
        <p className="section-subtitle">Best Seller</p>

        {products.length === 0 ? (
          <p>No AutoTools found.</p>
        ) : (
