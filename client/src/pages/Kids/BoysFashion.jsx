...
export default function BoysFashion() {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          "http://localhost:5000/api/products?category=kids&sub_category=Boys Fashion"
        );
        const data = await res.json();
        setProducts(data);
      } catch (error) {
        console.error("Error fetching Boys Fashion:", error);
      }
    };
    fetchProducts();
  }, []);
