// src/pages/MobilesTablets/CasesandCovers.jsx
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          "http://localhost:5000/api/products?category=mobiles_tablets&sub_category=Cases and Covers"
        );
        const data = await res.json();
        setProducts(data);
      } catch (error) {
        console.error("Error fetching Cases and Covers:", error);
      }
    };
    fetchProducts();
  }, []);
  ...
  return (
    <div className="page-container">
      ...
      <main className="main-content">
        <h2>Cases and Covers</h2>
        <p className="section-subtitle">Best Seller</p>

        {products.length === 0 ? (
          <p>No Cases and Covers found.</p>
        ) : (
